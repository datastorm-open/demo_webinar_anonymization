...
plt_original <- amBarplot("Departement", "somme", 
          data = cross_dia_dep, 
          horiz = T, show_values = T, main = "Pourcentage d'employés diabétiques par département avant anonymisation")

cross_dia_dep = data_anonym[, .(somme = round(sum(Cancer)/.N*100, 2)), by = c("Departement")]
plt_anonym <- amBarplot("Departement", "somme", 
                        data = cross_dia_dep, 
                        horiz = T, show_values = T, main = "Pourcentage d'employés diabétiques par département après anonymisation")
